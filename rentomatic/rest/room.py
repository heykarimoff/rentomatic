...
from rentomatic.repository import memory_repository as mr
...
def room():
    ...
    repo = mr.MemoryRepo([room1, room2, room3])
    use_case = uc.RoomListUseCase(repo)
    result = use_case.execute(request_object)

    return Response(
        json.dumps(result.value, cls=ser.RoomJsonEncoder), mimetype="application/json", status=STATUS_CODES[result.type]
    )
