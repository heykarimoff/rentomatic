...
from rentomatic.repository import memory_repository
...
def test_repository_list_without_parameters(room_dicts):
    repo = memory_repository.MemoryRepo(room_dicts)
...
def test_repository_list_with_code_equal_filter(room_dicts):
    ...
    repo_rooms = repo.list(filters={"code__eq": "eed76e77-55c1-41ce-985d-ca49bf6c0585"})
...
def test_repository_list_with_price_less_then_filter(room_dicts):
    repo = memory_repository.MemoryRepo(room_dicts)
    rooms = [r.Room.from_dict(i) for i in room_dicts]
...
def test_repository_list_with_price_greater_then_filter(room_dicts):
    repo = memory_repository.MemoryRepo(room_dicts)
    rooms = [r.Room.from_dict(i) for i in room_dicts]

    repo_rooms = repo.list(filters={"price__gt": 60})

    assert len(repo_rooms) == 1
    assert set([r.code for r in repo_rooms]) == {"fe2c3195-aeff-487a-a08f-e0bdc0ec6e9a"}
...
def test_repository_list_with_price_between_filter(room_dicts):
    ...
    repo_rooms = repo.list(filters={"price__gt": 50, "price__lt": 70})
